...
const SMART_ALERTS = [
  ...
  { id: 3, type: 'warning', icon: '🟡', label: 'Work Center Low Efficiency',     desc: 'Painting Center running at 48% — below 60% target.', time: '1 hr ago' },
  { id: 4, type: 'warning', icon: '🟡', label: 'Large Production Order Queued',  desc: 'MO-4001 (Executive Chair x20) requires urgent raw material approval.', time: '2 hr ago' },
];
        ...
        {activeAlerts.length > 0 && (
          <div className="mfg-alerts-bar">
            <div className="mfg-alerts-title">
              <Bell size={14} />
              Smart Alerts
              <span className="mfg-alerts-count">{activeAlerts.length}</span>
            </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="mfg-section">
            ...
            <div className="mfg-role-info">
              ...
              <span className="mfg-role-desc">
                {activeRole === 'Owner / Admin'
                  ? '→ Full overview: KPIs, alerts, analytics, audit trail, and all production orders.'
                  : activeRole === 'Floor Supervisor'
                  ? '→ Focus: Active & delayed orders, work center utilization, production progress.'
                  : '→ Focus: Material stock levels, raw material requests, pending procurement alerts.'}
              </span>
            </div>
                  ...
                  {PRODUCTION_TREND.map((d, i) => (
                    <div key={i} className="chart-bar-group" style={{ width: '52px' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', flex: 1, justifyContent: 'center' }}>
                        <div
                          className="chart-bar"
                          data-value={`Planned: ${d.planned}`}
                          style={{ height: `${(d.planned / maxProduced) * 100}%`, background: 'var(--color-outline-variant)', width: '16px' }}
                        />
                        <div
                          className="chart-bar"
                          data-value={`Produced: ${d.produced}`}
                          style={{ height: `${(d.produced / maxProduced) * 100}%`, background: 'var(--color-primary)', width: '16px' }}
                        />
                      </div>
